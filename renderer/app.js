...
function switchView(view) {
  ...
  if (view === 'advisor')  updateAdvisorView()
}
...
function bindEvents() {
  ...
  // AI Advisor
  $('btn-advisor-send').addEventListener('click', () => askAdvisor())
  $('advisor-input').addEventListener('keydown', e => { if (e.key === 'Enter') askAdvisor() })

  // Claude API key save on blur
  $('settings-claude-key').addEventListener('change', async () => {
    const key = $('settings-claude-key').value.trim()
    const s = await window.api.getSettings()
    await window.api.setSettings({ ...s, claudeApiKey: key })
    $('claude-key-status').textContent = key ? 'Key saved' : ''
    $('claude-key-status').style.color = 'var(--green-text)'
    updateAdvisorView()
  })
...
async function openSettingsModal() {
  ...
  // Pre-fill Claude API key
  if ($('settings-claude-key')) {
    $('settings-claude-key').value = settings.claudeApiKey || ''
    const status = $('claude-key-status')
...
async function updateAdvisorView() {
  ...
  if ($('settings-claude-key')) $('settings-claude-key').value = key
  $('advisor-no-key').style.display = key ? 'none' : ''
  $('advisor-chat').style.display = key ? 'flex' : 'none'
}

function appendAdvisorMsg(role, html) {
  const div = document.createElement('div')
  div.className = `advisor-msg advisor-msg-${role}`
  div.innerHTML = `<div class="advisor-msg-content">${html}</div>`
  $('advisor-messages').appendChild(div)
  $('advisor-messages').scrollTop = $('advisor-messages').scrollHeight
}
...
async function askAdvisor(prefill) {
  const input = prefill || $('advisor-input').value.trim()
  if (!input || _advisorBusy) return
  $('advisor-input').value = ''
  _advisorBusy = true

  // Remove suggestion chips after first message
  const chips = document.querySelector('.advisor-suggestions')
  if (chips) chips.remove()

  appendAdvisorMsg('user', esc(input))
  appendAdvisorMsg('ai', '<div class="advisor-typing">Thinking...</div>')

  const settings = await window.api.getSettings()
  const apiKey = settings.claudeApiKey
  if (!apiKey) {
    const last = $('advisor-messages').lastElementChild
    last.querySelector('.advisor-msg-content').innerHTML = 'No API key set — add your Claude key in Settings'
    _advisorBusy = false
    return
  }

  advisorHistory.push({ role: 'user', content: input })

  const result = await window.api.advisor.chat(advisorHistory, apiKey)
  const last = $('advisor-messages').lastElementChild

  if (result.error) {
    last.querySelector('.advisor-msg-content').innerHTML = `<span style="color:var(--red)">${esc(result.error)}</span>`
    advisorHistory.pop()
  } else {
    last.querySelector('.advisor-msg-content').innerHTML = markdownToHtml(result.content)
    advisorHistory.push({ role: 'assistant', content: result.content })
  }
...
// ── Deal Scanner ──────────────────────────────────────────────────────────────
const DEALS_EXTRACT_JS = `(function(){
  ...
  for(var i=0; i<items.length && out.length<50; i++){
    ...
    // Title: first meaningful span (skip "NEW LISTING", short labels)
    var title = '';
    for(var j=0; j<spans.length; j++){
      var s = spans[j];
      if(s.length > 15 && s !== 'Opens in a new window or tab' && !s.startsWith('$') && !/^\\d/.test(s) && !/product rating/.test(s)){
        title = s.replace(/^NEW LISTING\\s*/i,'').trim();
        break;
      }
    }
    if(!title) continue;
    ...
    for(var j=0; j<spans.length; j++){
      if(spans[j].charAt(0)==='$'){ priceText = spans[j]; priceNum = parseFloat(priceText.replace(/[^0-9.]/g,''))||0; break; }
    }
    if(!priceNum) continue;
    // Link
    var linkEl = el.querySelector('a[href*="/itm/"]');
    // Image
    var imgEl = el.querySelector('img[src*="ebayimg.com"]');
    var imgSrc = imgEl ? imgEl.src : '';
    // Shipping
    var shipping = '';
    for(var j=0; j<spans.length; j++){
      if(/delivery|shipping|Free/.test(spans[j]) && spans[j].length < 50){ shipping = spans[j]; break; }
    }
    // Seller
    var seller = '';
    for(var j=0; j<spans.length; j++){
      if(/positive/.test(spans[j])){ seller = spans[j]; break; }
    }
    out.push({ title:title, price:priceText, priceNum:priceNum, image:imgSrc,
               url:linkEl?linkEl.href:'', shipping:shipping, seller:seller });
  }
  return out;
})()`

function scrapeDealsWebview(wvId, url) {
  return new Promise((resolve) => {
    const wv = $(wvId)
    let done = false
    const finish = (val) => { if (done) return; done = true; clearTimeout(timer); resolve(val || []) }
    const timer = setTimeout(() => { console.log('[deals] webview timeout for', url); finish([]) }, 35000)
    ...
    const onReady = async () => {
      ...
      try {
        ...
        console.log(`[deals] ${wvId} loaded: title="${title}" url="${url}"`)
      } catch (e) { console.log(`[deals] ${wvId} title check error:`, e.message) }
      // Poll for .s-item elements
      for (let i = 0; i < 20 && !done; i++) {
        await new Promise(r => setTimeout(r, 1000))
        if (done) return
        try {
          const count = await wv.executeJavaScript(`document.querySelectorAll('ul.srp-results > li.s-card').length`)
          if (i === 0 || i === 5 || i === 10 || count > 0) {
            console.log('[deals] wv poll', i, 'cards:', count, 'for', wvId)
          }
          if (count >= 2) {
            const results = await wv.executeJavaScript(DEALS_EXTRACT_JS)
            console.log('[deals] extracted', results?.length || 0, 'items from', wvId)
            if (results && results.length) { finish(results); return }
          }
        } catch (e) { console.log('[deals] wv poll error:', e.message) }
      }
      finish([])
    }
...
async function runDealScan() {
  ...
  try {
    ...
    const soldUrl   = `https://www.ebay.com/sch/i.html?_nkw=${q}&_sacat=0&_from=R40&rt=nc&LH_Sold=1&LH_Complete=1&_ipg=60`
    const activeUrl = `https://www.ebay.com/sch/i.html?_nkw=${q}&_sacat=0&_ipg=60`

    // Scrape sold then active (webviews are rendered but clipped/invisible)
    const soldRaw = await scrapeDealsWebview('deals-wv-sold', soldUrl)
    const activeRaw = await scrapeDealsWebview('deals-wv-active', activeUrl)

    $('deals-loading').style.display = 'none'

    // Calculate market average from sold prices
    ...
    if (soldPrices.length) {
      ...
      avgSold = Math.round(((median + mean) / 2) * 100) / 100
    }

    // Show stats
    $('deals-avg-price').textContent    = soldCount ? '$' + avgSold.toFixed(2) : '—'
    $('deals-sold-count').textContent   = soldCount
    $('deals-active-count').textContent = activeCount
    $('deals-stats').style.display      = 'flex'

    // Find deals (active listings priced below sold average)
    let deals = []
    if (soldCount && activeCount) {
      deals = activeRaw
        ...
        .map(r => {
          ...
          return { ...r, avgSold, pctBelow: Math.round(pctBelow), dealScore }
        })
        .filter(r => r.pctBelow >= 15)
        .sort((a, b) => b.pctBelow - a.pctBelow)
    }

    $('deals-found-count').textContent = deals.length

    if (!deals.length) {
      const emptyEl = $('deals-empty')
      if (!soldCount && !activeCount) {
        emptyEl.querySelector('p').textContent = 'Could not fetch eBay data — try opening Market Lookup first to solve any captcha, then come back'
      } else if (!soldCount) {
        emptyEl.querySelector('p').textContent = 'No sold listings found — need sold data to calculate market value'
      } else {
        emptyEl.querySelector('p').textContent = `No deals found — all ${activeCount} active listings are at or above the $${avgSold.toFixed(2)} market average`
      }
